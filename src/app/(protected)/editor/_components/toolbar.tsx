      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Upload a file or paste an image link
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button type="submit" onClick={handleImageUrlSubmit}>
              Insert
            </Button>
          </DialogFooter>
        </DialogContent>
...
const HeadingLevelButton = () => {
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button className="h-7 min-w-7 shrink-0 flex items-center  rounded-sm hover:bg-neutral-200/80 px-1.5 overflow-hidden text-sm">
          <span className="truncate">{getCurrentHeading()}</span>
          <ChevronDownIcon className="ml-2 size-4 shrink-0" />
        </button>
      </DropdownMenuTrigger>
      <DropdownMenuContent className="p-1 flex flex-col gap-y-1">
        {headings.map(({ fontSize, lable, value }) => (
          <button
            key={value}
            style={{ fontSize }}
            onClick={() => {
...
const FontFamilyButton = () => {
  ...
  // when there are twp nested buttons we use as child to avoid error
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
